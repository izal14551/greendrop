...
<body class="bg-gray-100 min-h-screen p-6">
    <div class="pb-20">
        <div class="flex justify-between items-center mb-4">
            <h2 class="font-semibold text-lg">Daftar Peserta</h2>
            <a href="/peserta/tambah"
            ...
            foreach ($peserta as $p): ?>
                <div class="border rounded-lg p-3 shadow-sm">
                    <div class="flex justify-between items-center">
                        <h3 class="font-bold text-gray-700">#<?= $no++ ?> <?= $p['nama'] ?></h3>
                        <div class="text-xs">
                            <a href="/peserta/edit/<?= $p['id'] ?>" class="text-white hover:underline bg-blue-600 p-2 rounded">Edit</a> |
                            <a href="/peserta/delete/<?= $p['id'] ?>" class="text-white hover:underline bg-red-600 p-2 rounded"
                                onclick="return confirm('Yakin hapus?')">Hapus</a>
                        </div>
                    </div>
                    <p class="text-sm text-gray-600">NIK: <?= $p['nik'] ?></p>
                    <p class="text-sm text-gray-600">Alamat: <?= $p['alamat'] ?></p>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t shadow-md flex justify-around items-center py-2">
        ...
        <a href="/peserta" class="flex flex-col items-center text-blue-600">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
